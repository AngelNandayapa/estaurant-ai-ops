...
SUPERMERCADOS = frozenset([
    "SORIANA", "CHEDRAUI", "HEB", "WALMART", "OXXO",
    "SAMS", "SAM'S", "COSTCO", "BODEGA AURRERA"
])
...
def procesar_factura(message):
    ...
    try:
        ...
        # ── FIX MEDIUM #2: dropna=False so rows with no date aren't silently lost ──
        for mes, datos_mes in df_nuevo.groupby('Mes_Pestaña', dropna=False):
        ...
        total_ticket = pd.to_numeric(df_nuevo['Gasto Total'], errors='coerce').fillna(0).sum()
